...
import { localDayStartUnix, parseAppTimestamp, toUnixSeconds } from "@/lib/time/timestamp";
...
function candleCloseByDay(candles: FinnhubCandle | null): Map<number, number> {
  ...
  for (let i = 0; i < candles.t.length; i++) {
    ...
    const day = localDayStartUnix(new Date(t * 1000).toISOString());
    map.set(day, close);
  }
  return map;
}
  ...
  return [...calls]
    ...
    .map((c) => {
      ...
      return {
        ...
        basisPrice: basis,
        calledAt: localDayStartUnix(c.called_at),
        closedAt: c.closed_at ? localDayStartUnix(c.closed_at) : null,
...
export async function buildCumulativeReturnMarkToMarketSeries(
  ...
  const today = localDayStartUnix(new Date());
  ...
  for (let day = firstDay; day <= today; day += 86400) {
    let cumulative = 0;
    let hasData = false;
