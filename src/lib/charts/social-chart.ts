...
/** Detect image MIME from base64 magic bytes for Resvg embedded images. */
function logoDataUri(base64: string): string {
...
export function renderSocialChartSvg(payload: SocialChartPayload): string {
  ...
  if (fueledMarker && candles.length > 0) {
    ...
    markerSvg += `<line x1="${callX}" y1="${chartY}" x2="${callX}" y2="${chartY + chartH}" stroke="${T.fueled}" stroke-width="1" stroke-dasharray="3 7" opacity="0.2"/>`;
    markerSvg += `<circle cx="${callX}" cy="${y}" r="7" fill="${T.fueled}"/>`;
    markerSvg += `<circle cx="${callX}" cy="${y}" r="7" fill="none" stroke="#ffffff" stroke-width="1.5" opacity="0.5"/>`;
  }
  ...
  const logoRenderH = 72;
  const logoRenderW = Math.round(logoRenderH * SOCIAL_LOGO_ASPECT);
  const logoX = chartX + chartW - logoRenderW - 12;
  const logoY = chartY + chartH - logoRenderH - 12;

  const logoImg = payload.logoBase64
    ? `<image href="${logoDataUri(payload.logoBase64)}" x="${logoX}" y="${logoY}" width="${logoRenderW}" height="${logoRenderH}" preserveAspectRatio="xMaxYMax meet"/>`
    : text(logoX + logoRenderW, logoY + 48, "PortFuel PRO", {
        size: 14,
        weight: 700,
        fill: T.textBright,
        anchor: "end",
  ...
  const milestoneBadge = badgeLabel
    ? `<rect x="${perfX - badgeW}" y="${metaY - 20}" width="${badgeW}" height="26" rx="13" fill="${T.accentSoft}" stroke="${T.accent}" stroke-width="1"/>
       ${text(perfX - badgeW / 2, metaY - 3, badgeLabel, { size: 8, weight: 700, fill: T.textBright, anchor: "middle" })}`
    : "";
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  <defs>
    ...
    <radialGradient id="chartGlow" cx="78%" cy="92%" r="50%">
  ...
  <rect x="0" y="0" width="4" height="${H}" fill="url(#accentBar)"/>
  ${titleBlock}
  ...
  <rect x="${pad}" y="${chartY}" width="${panelW}" height="${chartH}" rx="10" fill="${T.chartBg}" stroke="${T.chartBorder}" stroke-width="1"/>
  <rect x="${pad}" y="${chartY}" width="${panelW}" height="${chartH}" rx="10" fill="url(#chartGlow)"/>
