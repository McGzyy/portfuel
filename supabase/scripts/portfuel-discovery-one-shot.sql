...
-- 9) Paper-track shadow calls (20260715100000_discovery_shadow_calls.sql)
...
begin
  if exists (
    ...
    insert into supabase_migrations.schema_migrations (version, name)
    values ('20260715100000', 'discovery_shadow_calls')
    on conflict (version) do nothing;
  end if;
end $$;

select to_regclass('public.desk_discovery_shadow_calls') as shadow_table;
